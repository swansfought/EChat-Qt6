...
//    this->setAttribute(Qt::WA_DeleteOnClose);
    ...
    QAction *actAddFriend = new QAction("加好友/群聊",this);
    QAction *actCreateGroup = new QAction("创建群聊",this);
    connect(actAddFriend,&QAction::triggered,this,[=]{ //跳转到加好友/群界面
    ...
    connect(actCreateGroup,&QAction::triggered,this,[=]{ //跳转到加好友/群界面
        on_btn_contact_clicked();
    ...
    menu->addAction(actAddFriend);
    menu->addAction(actCreateGroup);
    ...
    //头像选择框关闭
    connect(ui->line_realTimeSearch,&QLineEdit::textChanged,this,[=]{ closeHeadPicture(); });
    connect(ui->line_groupName,&QLineEdit::textChanged,this,[=]{ closeHeadPicture(); });
...
{
    //搜索EC号/手机号码：纯数字
    QRegularExpression numReg(NUM_REG);
    ui->line_seekInput->setValidator(new QRegularExpressionValidator(numReg,this));
    ui->line_phone->setValidator(new QRegularExpressionValidator(numReg,this));

    //密码：英文大小写+数字
    QRegularExpression pwdReg(PWD_REG);
    ui->line_newPassword->setValidator(new QRegularExpressionValidator(pwdReg,this));
    ui->line_sureNewPassword->setValidator(new QRegularExpressionValidator(pwdReg,this));

    //邮箱
    QRegularExpression emailReg(EMAIL_REG);
    ui->line_email->setValidator(new QRegularExpressionValidator(emailReg,this));

    //地区：英文+汉字+数字
    QRegularExpression classReg(ZH_EN_NUM_REG);
    ui->line_region->setValidator(new QRegularExpressionValidator(classReg,this));
//    ui->line_nickname->clear();
//    ui->text_selfSay->clear();
