...
void UiLogin::mousePressEvent(QMouseEvent *event)
{
    ...
    offy = event->globalPosition().y() - this->pos().y();
...
{
    //账户/验证结果：纯数字
    QRegularExpression numReg(QString(NUM_REG));
    ui->com_loginAccount->setValidator(new QRegularExpressionValidator(numReg,this));
    ui->line_verifyResult->setValidator(new QRegularExpressionValidator(numReg,this));

    //密码：英文大小写+数字
    QRegularExpression pwdReg(QString(PWD_REG));
    ui->line_loginPwd->setValidator(new QRegularExpressionValidator(numReg,this));
    ui->line_registerPwd->setValidator(new QRegularExpressionValidator(numReg,this));
}
